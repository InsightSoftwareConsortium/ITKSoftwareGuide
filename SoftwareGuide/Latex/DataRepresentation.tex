...
capability to analyze visual light using three sprectral
...
sensitive to color~\cite{Gray2003}. The \emph{rods} on the other hand provide
no color discrimination and favor high resolution and high
sensitivity\footnote{The human eye is capable of perceiving a single photon}.
A fifth type of receptors, the \emph{ganglion cells}, also known as
circadian\footnote{Circadian refers to the cycle of day and night} receptors
are sensitive to the lighthing conditions that differentiate day from night.
These receptors evolved as a mechanism for syncronizing the physiology with the
time of the day. Cellular controls for circadian rithms are present in every
cell of an organism and are known to be exquisitively
precise~\cite{Lodish2000}.  

The RGB space has been constructed as a representation of a physiological
response to light by tthe three types of \emph{cones} in the human eye. RGB is
...
as an artificial construct for color comparision in the sense that
...
ColorB.  However, we must be aware that (at least in emitted light) it is not
...
to emmitted light like in computer screens, the physical behavior of color
allows for subtraction. This is because strictly speaking the objects that you
...
concept from the ones of \doxygen{Vectors} in space. For this reason, the
...
will have accesss to the set of operations that are defined in Vector spaces.
