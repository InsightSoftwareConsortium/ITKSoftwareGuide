...
to register a parametric representation of a hearth with an individual's CT
...
char values (e.g., \code{UC}). Typically just a few common types are
...
based on these basic options, are defined by the \code{*.wrap} files in the
...
\code{2;3} indicating support for 2- and 3-dimensional images.
...
The environment for access to the \code{itk} Python module is best configured
...
directory ITK was built in. Create a symbolic link to the \code{WrapITK.pth} file
within the \code{lib/python2.7/site-packages} directory of Python's virtual install
directory.
