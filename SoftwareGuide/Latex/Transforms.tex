...
In the Insight toolkit, \doxygen{Transform} objects encapsulate the mapping
...
\doxygen{Transform} classes provide different methods for mapping each one of
...
\index{itk::Transform!SetParameters()} Typically each transform class has
...
\doxygen{Array} of \code{double} to facilitates communication with generic
optimizers. In the case of \doxygen{Euler2DTransform}, the transform is also
...
Where $\{p_i\}$ are the transform parameters and $\{x_i\}$ are the
coordinates of the output point.  Within this framework, the Jacobian is
represented by an
\doxygen{Array2D} of \code{double}s and is obtained from the transform by method
...
The translation transform \doxygen{TranslationTransform} is probably the
simplest yet one of the most useful transformations.  It maps all points by
adding a constant values to their coordinates.  Vector and covariant vectors
remain unchanged under this transformation since they are not associated with
a particular position in space. Translation is the best transform to start a
...
the images. Translations also have the advantages of being fast to compute
...
The \doxygen{ScaleTransform} transform represents a simple scaling of the
...
Scaling is appears to be a simple transformation but actually there are a
number of issues to keep in mind when using different scale factors along
every dimension. It has subtle effects---for example, when computing image
derivatives. Since derivatives are represented by covariant vectors their
values are modified in a non-intuitive way by scaling transforms.
...
diminishs as the factor grows. In other words, a scale factor variation of
...
\doxygen{AffineTransform}, have hidden scaling parameters and are therefore
subject to the same vulnerabilities of the
\doxygen{ScaleTransform}.
...
rotations do not form a vector space and can not be managed as linear
...
deviate the trajectories of descent making optimization slower and more
...
of the optimizers. The main characteristics of the \doxygen{Euler2DTransform}
...
\doxygen{Euler2DTransform} always use the origin of the coordinate system as
the center of rotation. This distintion is quite important in image
registration since ITK images usually have their origin in the corner of the
image rather than the middle.  Rotational mis-registrations however usually
exist as rotations around the center of the image, or at least as rotations
...
user-especified vector.

Similar to the \doxygen{Euler2DTransform} this transform suffers from the
...
after the rotation. The main features of the
\doxygen{CenteredRigid2DTransform} are presented in
Table~\ref{tab:CenteredRigid2DTransformCharacteristics}.
...
A similarity transform \doxygen{Similarity2DTransform} preserves angles. It
can be seen as a rigid transform plus an isotropic scaling factor. In its
$2D$ implementation, the four parameters of this transformation combine the
characteristics of the \doxygen{ScaleTransform} and
\doxygen{Euler2DTransform} relating to the non-linearity of the parameter
space and the non-uniformity of the measurement units. Gradient-descent
...
The main features of the \doxygen{Similarity2DTransform} are presented in
...
and perform another set of iterations updating only translations.
...
\doxygen{Similarity2DTransform} when used with gradient-descent optimizers.

To address these concerns, the
\doxygen{QuaternionRigidTransformGradientDescentOptimizer} was introduced into
the toolkit.  This specialized optimizer implements a variation of a
...
resulting set of transform parameters are mapped back into the permissible
set of parameters. In practice this comes down to normalizing the newly
computed quaternion to make sure that the transformation remains rigid and no
scaling is applied.  The main characteristics of the
\doxygen{QuaternionRigidTransform} are presented in
...
length. The main characteristics of the versor transform
...
allow the creation of smooth rotational trajectories and for this reason they
...
\doxygen{QuaternionRigidTransform}. It can be seen as an
...
with respect to the \doxygen{QuaternionRigidTransform} is that it exposes
only siz parameters, three for the versor components and three for the
translational components. This reduces the search space for the optimizer 
to six dimensions instead of the seven dimensional used by
\doxygen{QuaternionRigidTransform}. The main features of this transform
...
for image registration. Its main advantage comes from the fact of its being
...
The set of \doxygen{AffineTransform} coefficients can actually be represented
...
dimensionality of the search space implies also a high computational cost
during the computation of cost-function derivatives. The best compromise in
...
This difference in scale again makes it necessary to take advantage of the
...
optimization iterations would typically be focused on removing large
translations. This task could rather be accomplished by a translation
...
\doxygen{AffineTransforms} can be a challenging task since it is difficult to
represent the coefficients in a meaningful way.  A simple print out of the
transform coefficients generally do not offer a clear picture of the current
