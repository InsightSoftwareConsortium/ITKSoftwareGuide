...
In the toolkit, \doxygen{Transform} objects encapsulate the mapping of points
...
\itkcaption[Geometrical representation objects in ITK]{Geometrical
...
Additional uses of the \doxygen{Point}, \doxygen{Vector} and \doxygen{CovariantVector}
...
\index{itk::Transform!SetParameters()} Typically each transform class have
several methods for setting its parameters.  For example,
\doxygen{Euler2DTransform} provide methods for separately setting the offset,
...
\doxygen{Array} of \code{double} to allow communication with generic
...
process. It is just a \code{NULL} operation. The main characteristics of the
...
\end{tabular}
\end{center}
\itkcaption[Translation Transform Characteristics]{Characteristics of the Translation Transform
...
\end{tabular}
\end{center}
\itkcaption[Scale Transform Characteristics]{Characteristics of the Scale Transform
...
example the \doxygen{AffineTransform}, have hidden scaling parameters and are henceforth
subject to the same vulnerabilities of the \doxygen{ScaleTransform}.
...
\end{tabular}
\end{center}
\itkcaption[Euler2D Transform Characteristics]{Characteristics of the Euler2D Transform
...
\end{tabular}
\end{center}
\itkcaption[CenteredRigid2D Transform Characteristics]{Characteristics of the CenteredRigid2D Transform
...
difference between this transform and the \doxygen{Euler2DTransform} is that here
...
As the \doxygen{Euler2DTransform} this transform suffers from the difference in
...
\end{tabular}
\end{center}
\itkcaption[Similarity2D Transform Characteristics]{Characteristics of the Similarity2D Transform
...
\doxygen{ScaleTransform} and \doxygen{Euler2DTransform} concerning the non-linearity of the
...
\end{tabular}
\end{center}
\itkcaption[QuaternionRigid Transform Characteristics]{Characteristics of the QuaternionRigid Transform
...
and hence raise the same concerns of the \doxygen{Similarity2DTransform} when used
with gradient-descent optimizers.

As a solution, the \doxygen{QuaternionRigidTransformGradientDescentOptimizer}
...
applied.  The main characteristics of the \doxygen{QuaternionRigidTransform} are 
...
\end{tabular}
\end{center}
\itkcaption[Versor Rigid3D Transform Characteristics]{Characteristics of the VersorRigid3D Transform
\label{tab:VersorRigid3DTransformCharacteristics}}
\end{table}

This transform is a close variant of the \doxygen{QuaternionRigidTransform}. It
can be seen as an \doxygen{VersorTransform} plus a translation defined by a
vector. It general terms it implements a rigid transform in $3D$.  The
advantage of this class with respect to the \doxygen{QuaternionRigidTransform} is
...
\doxygen{QuaternionRigidTransform}. The main characteristics of this transform
...
\end{tabular}
\end{center}
\itkcaption[Affine Transform Characteristics]{Characteristics of the Affine Transform
\label{tab:AffineTransformCharacteristics}}
\end{table}


The \doxygen{AffineTransform} is one of the most popular transformations in
