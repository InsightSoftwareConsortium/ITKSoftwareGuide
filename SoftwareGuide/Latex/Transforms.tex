...
rapidly the rotational component of a more generic miss-registration.  It is
...
representation for rotations in $3D$ space. Sequences of Versors allow to
create smooth rotational trajectories and for this reason they behave stably
...
in a vector space of dimensions $(N+1) \times N$. This makes possible for
optimizers to be used appropriately on this search space. However the high
dimension of the search space implies also a high computational cost during the
...
different dynamic range than the translation coefficients. Coefficients in the 
...
This difference in scale make necessary again to take advantage of the
...
miss-registration. Otherwise it will incur in an overwhelming computational
...
\code{AffineTransform} can be a challenging task since it is difficult to
