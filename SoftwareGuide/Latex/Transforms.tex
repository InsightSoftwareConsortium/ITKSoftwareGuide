...
For example, \doxygen{Euler2DTransform} provide methods for separately setting the
...
\doxygen{Array}<double> to allow communication with generic optimizers. In the
case of \doxygen{Euler2DTransform}, the transform is also defined by three
...
\doxygen{Array2D}<double> and is obtained from the transform by method
...
can be seen as a the \doxygen{VersorTransform} plus a translation defined by a
