...
In ITK, \doxygen{ImageToImageMetric} objects measure quantitatively how well
...
\doxygen{MeanSquaresImageToImageMetric} computes the mean square pixel-wise difference in intensity
...
\doxygen{NormalizedCorrelationImageToImageMetric} computes pixel-wise
...
\index{itk::MeanReciprocalSquareDifferenceImageToImageMetric|textbf}
\doxygen{MeanReciprocalSquareDifferenceImageToImageMetric} computes pixel-wise differences and add them 
...
A variety of functions can be used as the smoothing kernel with the requirement
that it be smooth, symmetric, has zero mean and integrates to one. For example,
boxcar, Gaussian and B-spline functions are suitable candidates.  A smoothing
parameter is used to scale the kernel function.  The larger the smoothing
parameter, the wider the kernel function used and hence the smoother the
density estimate. If the parameter is too large, features such as modes in the
density will get smoothed out.  On the other hand, if the smoothing parameter
is too small, the resulting density may be too noisy. The estimation is given 
...
\doxygen{MutualInformationImageToImageMetric} that follows the method specified
...
the class  \doxygen{MattesMutualInformationImageToImageMetric}.
