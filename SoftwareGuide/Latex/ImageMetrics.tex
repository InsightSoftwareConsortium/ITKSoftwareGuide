...
where $H(f,m)$ is the count on the histogram bin identified with fixed image
...
The correlation coefficient is computed from the Joint histogran as
...
Where $H(f,m)$ is the histogram count for the bin identified with the fixed
image intensity $f$ and the moving image intensity $m$. The values
$\overline{f}$, $\overline{m}$ are the mean values of the fixed and moving
...
coefficient is $1$,
which would indicate a perfect straight line in the histogram.
