...
workspace named \code{ITK.sln} (if using Visual Studio) from the binary
...
your project. Under UNIX, you can \code{cd} to your newly created build
directory and type ``\code{ccmake ../HelloWorldITK}''. Note the
...
where ITK was built, \code{ITK\_DIR}.  The ITK binary directory will contain a
file named \code{ITKConfig.cmake} generated during the configuration process
...
After generating the build files, the projects can be built again either with
\code{make} in the binary directory on UNIX or by loading the
workspace named \code{HelloWorldITK.sln} (if using Visual Studio) from the binary
directory specified in the CMake GUI, right clicking on \code{ALL\_BUILD} and
clicking \code{Build}.
...
line by starting the \code{cmd.exe} shell first.
