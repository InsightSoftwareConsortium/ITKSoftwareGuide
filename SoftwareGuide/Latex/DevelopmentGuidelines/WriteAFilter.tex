...
\begin{itemize}
        ...
        operators on, and outputs data\index{data processing pipeline}.

        \item A \textbf{filter}\index{filter}, or \textbf{process object}\index{process object}, has one or more
        inputs, and one or more outputs.

        \item A \textbf{source}\index{source}, or source process object, initiates the data
        processing pipeline, and has one or more outputs.

        \item A \textbf{mapper}\index{mapper}, or mapper process object, terminates the
        ...
        \item A \textbf{data object}\index{data object} represents and provides access to
        data. In ITK, the data object (ITK class \doxygen{DataObject}) is
        typically of type \doxygen{Image} or \doxygen{Mesh}.

        \item A \textbf{region}\index{region} (ITK class \doxygen{Region}) represents a
        piece, or subset of the entire data set.

        \item An \textbf{image region}\index{image region} (ITK class \doxygen{ImageRegion})
        represents a structured portion of data. ImageRegion is implemented
        using the \doxygen{Index} and \doxygen{Size} classes

        \item A \textbf{mesh region}\index{mesh region} (ITK class \doxygen{MeshRegion})
        represents an unstructured portion of data.

        \item The \textbf{LargestPossibleRegion}\index{LargestPossibleRegion} is the theoretical single,
        ...
        \item The \textbf{BufferedRegion}\index{BufferedRegion} is a contiguous block of memory
        ...
        \item The \textbf{RequestedRegion}\index{RequestedRegion} is the piece of the dataset that a
        ...
        \item The \textbf{modified time}\index{modified time} (represented by ITK class
        ...
        \item \textbf{Downstream}\index{pipeline!downstream} is the direction of dataflow, from sources
        to mappers.

        \item \textbf{Upstream}\index{pipeline!upstream} is the opposite of downstream, from mappers
        to sources.

        \item The \textbf{pipeline modified time}\index{pipeline!modified time} for a particular data
        object is the maximum modified time of all upstream data objects and
        process objects.

        \item The term \textbf{information}\index{pipeline!information} refers to metadata that
...
{Relationship between DataObject and ProcessObject\label{fig:DataPipeLineOneConnection}}.
\end{floatingfigure}
...
(see Section~\ref{sec:UsingStandardMacros}
on page~\pageref{sec:UsingStandardMacros}).
