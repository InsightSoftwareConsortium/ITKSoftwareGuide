...
\doxygen{CovariantVector} classes have been discussed in the Data
Representation chapter of Book 1.  Each one of these classes behaves
...
One of the classes that deserves further comments is the \doxygen{Vector}. This
...
\doxygen{FixedArray} as the base class of the \doxygen{Point}, the \doxygen{Vector}
and the \doxygen{CovariantVector} was a design decision based on the decision to
...
Each transform class typically has several methods for setting its parameters.%
\index{itk::Transform!SetParameters()}
For example, \doxygen{Euler2DTransform} provides methods for specifying the offset,
...
classes is the computation of their Jacobians.\index{itk::Transform!GetJacobian()}
In general, metrics require
...
in the outcome of the process. The main
...
Three rotational parameters are non-linear and do not behave
...
The scaling and rotational spaces are non-linear and do not
...
This transform does not provide functionality for mapping Vectors nor
...
\doxygen{LBFGSBOptimizer}. The use of this transform was proposed in the
