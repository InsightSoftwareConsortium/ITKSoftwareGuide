...
ITK, \code{SmartPointer}s can be allocated on the program stack, and are
automatically deleted when the scope that the \code{SmartPointer} was created
...
Note that in ITK SmartPointer's are always used to refer to instances of
...
\code{itk:Image} and \code{itk::Mesh}, both of which are subclasses of
\code{itk::DataObject}. In ITK, data objects are classes that are meant to
...
\index{itk::Image|textbf}

\code{itk::Image} represents a \emph{n}-dimensional, regular sampling of
...
pixel, value in ITK is arbitrary---a template parameter \code{TPixel}
...
\item \code{RequestedRegion}---the portion of the reqion requested by a 
filter or other class when operating on the image.
\end{enumerate}

\index{itk::Mesh|textbf} 

\code{itk::Mesh} represents a \emph{n}-dimensional, unstructured grid. The
...
\code{itk::Mesh} is a subclass of \code{itk::PointSet}. The \code{PointSet}
class can be used to represent point clouds or randomly distributed
landmarks, etc. The \code{PointSet} class has no associated topology.


\section{Data Processing Pipeline}
\label{sec:DataProcessingPipeline}

\index{data processing pipeline|textbf}

\index{process object|textbf} 
\index{source|textbf}
\index{reader|textbf} 
\index{filter|textbf} 
\index{mapper|textbf} 
...
In this example the source object \code{itk::RandomImageSource} is connected
to the \code{itk::ShrinkImageFilter}, and the shrink filter is connected to
the mapper \code{itk::ImageFileWriter}. When the \code{Update()} method is
...
\begin{enumerate}
  \item \code{gccxml} is a modified version of the GNU compiler gcc that
    produces an XML description of an input C++ program.
  \item  \code{CABLE} processes XML information from \code{gccxml} and produces
    additional input to the next tool (i.e., \code{CSWIG} indicating what is
    to be wrapped.
  \item \code{CSWIG} is a modified version of SWIG that has has SWIG's usual
    parser replaced with an XML parser (XML produced from \code{CABLE} and
    \code{gccxml}.) \code{CSWIG} produces the appropriate language bindings
    (either Tcl or Python). (Note: since \code{SWIG} is capable of producing
...
translation from C++, with the differences being the
particular syntactical requirements of each language. For example, in the
directory \code{Testing/Code/Algorithms} directory, the test 
\code{itkCurvatureFlowTestTcl2.tcl} has a code fragment that appears as follows:
\small
\begin{verbatim}
...
instantiated using an output and output image type of two-dimensional float
