...
\begin{description}
	...
	\item[Wrapping.] ITK uses a unique, powerful system for producing
        ...
        The parsed code is in turn represented using XML that is then
...
Reference counting is implemented through a \code{Register()}/\code{Delte()}
...
ITK uses the \code{VNL} numerics library to provide an environment for numerical
programming that combines the ease of use of packages like Mathematica and
...
ITK extends the functionality of \code{VNL} by including interface
classes between \code{VNL} and ITK proper.
