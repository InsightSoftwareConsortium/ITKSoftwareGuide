...
\doxygen{itk::ExceptionObject}. (In fact in ITK all exceptions should be derived
...
process. (Note that events in ITK are subclasses of \doxygen{itk::EventObject}; look
...
by ITK. For example, the class \code{MultiThreader} provides support for
...
method \code{ImageSource} (a superclass for most image processing filters)
...
\doxygen{itk::Image} represents an \emph{n}-dimensional, regular sampling of
...
topology of the mesh is represented by a set of \emph{cells} defined by a type and
...
modified data, and mappers accept data for output either to a file or
...
\begin{enumerate}
  ...
  \item  CABLE processes XML information from \code{gccxml} and produces
  ...
  \item CSWIG is a modified version of SWIG that has has SWIG's usual
    ...
    \code{gccxml}.) CSWIG produces the appropriate language bindings
