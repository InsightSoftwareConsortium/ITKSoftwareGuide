...
This purpose of this chapter is to assist developers create their own
...
writer will have to implement either a \code{GenerateData()}
(non-threaded) or \code{ThreadedGenerateData()} method. (See ``'' for
more information.)
...
\begin{enumerate}
        ...
        ``cache''. If the cache is allowed to remain (\code{ReleaseDataOff()}) 
...
events. (The threadId is passed as argument into ThreadedGenerateData()). 
If more than one thread tries to write to the same place at the same time, 
the program can behave badly, and even crash.
