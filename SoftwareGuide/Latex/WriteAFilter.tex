...
\begin{itemize}
        ...
        type \doxygen{Image} or \doxygen{Mesh}.
        \index{data object}

        \item A \textbf{region} (ITK class \doxygen{Region}) represents a piece,
        ...
        \item A \textbf{mesh region} (ITK class \doxygen{MeshRegion}) represents
...
{Relationship between \doxygen{DataObject} and \doxygen{ProcessObject}.
\label{fig:DataPipeLineOneConnection}}
\parpic(6cm,4cm)[r]{\includegraphics[width=5cm]{DataPipelineOneConnection.eps}}
...
on output. The superclass \doxygen{ImageToImageFilter} is a convenience class that
...
class defining the methods required by most all ITK objects: \code{New()},
\code{PrintSelf()}, and protected constructor, copy constructor, delete, and
...
\code{ThreadedGenerateData()} method. (See ``'' for more information.)
...
(see \doxygen{ImageSource}, a superclass of \doxygen{ImageToImageFilter})
...
\code{ProcessObject::Update()} method invocation. This method is simply delegated to
the output of the filter, invoking the \code{DataObject::Update()} method. Note that
...
\code{UpdateOutputInformation()}). This virtual method invokes
\code{DataObject::CopyInformation()}. The default behavior is for the
\code{GenerateOutputInformation()} to copy the meta data describing the input
...
The function of \code{PropagateRequestedRegion()} is, given a request for data (the
...
The \code{ProcessObject::PropagateRequestedRegion()} method invokes three methods
...
\doxygen{GibbsPriorFilter} is an example of a filter that needs to invoke
\code{EnlargeOutputRequestedRegion()}. The designer of this filter decided that
the filter should operate on all the data. Note that a subtle interplay between
this method and \code{GenerateInputRequestedRegion()} is occurring here. The
default behavior of \code{GenerateInputRequestedRegion()} (at least for
\doxygen{ImageToImageFilter}) is to set the input RequestedRegion to the output's
...
\code{ThreadedGenerateData()} method. For example, a \doxygen{ImageToImageFilter}
