...
\itkcaption[Cmake user interface]{CMake interface. Left) \texttt{ccmake}, the UNIX
...
may want to disable the compilation of the testing and examples. This is done
with the variables \code{BUILD\_TESTING=OFF} and \code{BUILD\_EXAMPLES=OFF}.
The examples distributed with the toolkit are a helpful resource for learning
how to use ITK components. The testing section includes a large number of small 
programs that exercise the capabilities of ITK classes. Due to the number of
tests, enabling the testing option will considerable increase the buliding
time.  It is not desirable to enable this option for a first built of the
toolkit.

An additional resource is available at the \code{InsightApplications} module,
which contains multiple applications incorporationg GUIs and different levels
...
depending on the performance of your system. If you enabled testing as part of
the normal built process, about 500 small test programs will be compiled
greatly increasing the build time. These test programs will verify that the
basic components of ITK compile and run correctly on your system.
