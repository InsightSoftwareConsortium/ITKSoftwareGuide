...
The first argument is a smart pointer to the \doxygen{Image} to iterate. The
second argument is an \doxygen{ImageRegion} that specifies the \emph{iteration
...
subregion of the image within the current \code{BufferedRegion}.  See
...
All standard ITK iterator operations defined in
...
require an argument that defines the extent of the neighborhood.  Neighborhood
...
Figure~{\ref{fig:NeighborhoodIteratorFig2} shows the relationship
between the radius of the iterator and the size of the neighborhood for a
...
In figure~\ref{fig:NeighborhoodIteratorFig2}, the linear array index is shown
at the top of each pixel.
...
In figure~\ref{fig:NeighborhoodIteratorFig2}, the offset it given below the
linear array index in each neighborhood pixel.
