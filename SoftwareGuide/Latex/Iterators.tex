...
\index{Iterators!operator-=()}
\item \textbf{\code{operator-=( Offset \code{0} )}} Moves the iterator to 
the pixel position at the current index minus offset \code{o}.
...
in \doxygen{Neighborhood N}, which must be the same size and shape as the
iterator.
...
\item \textbf{\code{itk::IndexType GetIndex()}} Return the \doxygen{Image}
index of the center pixel of the neighborhood iterator.

\item \textbf{\code{itk::ImageIndex GetIndex(itk::Offset o)}} Return the
\doxygen{Image} index of the pixel at offset \code{o} from the neighborhood 
center.

\item \textbf{\code{itk::ImageIndex GetIndex(unsigned int i)}} Return the
\doxygen{Image} index of the pixel at array position \code{i}.
...
of the standard \doxygen{NeighborhoodIterator} that supports an
...
of \doxygen{NeighborhoodIterator}.
