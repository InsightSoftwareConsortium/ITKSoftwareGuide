...
This chapter introduces the \emph{image iterator}, an important
...
There is a const and a non-const version of most ITK image iterators. A non-const
iterator cannot be instantiated on a non-const image pointer.  Const versions
of iterators may read, but may not write pixel values. 
...
the iterator will reference, or ``point to'', one pixel location in the ND
image.  \emph{Forward iteration} goes from the beginning of the iteration
...
\itkcaption[ITK image iteration]{Normal path of an iterator through a 2D image.  The
...
\begin{itemize}
\index{Iterators!operator+=()}
\item \textbf{\code{operator+=( itk::Offset \code{o} )}} Moves the iterator to the pixel
position at the current index plus offset \code{o}.

\index{Iterators!operator-=()}
\item \textbf{\code{operator-=( Offset \code{0} )}} Moves the iterator to the pixel
position at the current index minus offset \code{o}.
...
The \code{Get} and \code{Set} methods are inlined and optimized for speed so
that their use is equivalent to dereferencing the image buffer directly.  There
are a few common cases, however, where \code{Get} and \code{Set} cannot do
incur a penalty. Consider the following code, which fetches, modifies, and then
...
The \code{Value()} method can be used as either an lval or an rval in an expression.  It
...
\doxygen{ImageRegionIterator} that are designed make common image processing
...
by calling the \code{GetRadius} method.  Some other methods provide some useful
...
neighborhood pixel can be dereferenced by its integer index (top) or its offset from
the center (bottom).  The center pixel of each iterator is shaded.}
...
\item \textbf{\code{unsigned long Size()}} Returns the size in number of pixels of the
neighborhood.
...
offset from the neighborhood center.  The upper-left-forward corner of a $3x3x3$
neighborhood, for example, can be described by offset $(-1, -1, -1)$. The
bottom-right-back corner of the same neighborhood is at offset $(1, 1, 1)$.  In
figure~\ref{fig:NeighborhoodIteratorFig2}, the offset from center is shown
...
A neighborhood-based calculation in a neighborhood close to an image boundary
may require data that falls outside the boundary.  The iterator in
figure~\{fig:NeighborhoodIteratorFig1}, for example, is centered on a boundary
pixel such that three of its neighbors actually do not exist in the image.
When the extent of a neighborhood falls outside the image, pixel values for
missing neighbors are supplied according to a rule, usually chosen to satisfy
the numerical requirements of the algorithm.  A rule for supplying
out-of-bounds values is called a \emph{boundary condition}.
...
\code{b} instead. Object \code{b} should not be deleted until it has been
released by the iterator.  This method can be used to change iterator behavior
at run-time.
...
of the standard \code{itk::NeighborhoodIterator} that supports an
...
\code{itk::NeighborhoodIterator}.  Together with its \code{const} version,
...
of \doxygen{itk::NeighborhoodIterator}.
...
implement some binary image morphology algorithms (see \cite{Gonzales1993},
