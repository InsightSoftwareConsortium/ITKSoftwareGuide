...
\doxygen{itk::Image}s instantiated with different combinations of pixel type, pixel
...
advantages over using the \doxygen{itk::Image} interface directly.  Code is more
...
The first argument is a smart pointer to an \doxygen{itk::Image}.  This is the
image to iterate over. The second argument is an
\doxygen{itk::ImageRegion} that specifies the \emph{iteration region}.  The
...
iterator cannot be instantiated with a const \doxygen{itk::Image} smart pointer.
...
\doxygen{itk::ImageRegionIterator}.  For more examples, see
sections~\ref{sec:ImageIterators}--\ref{sec:NeighborhoodIterators}.
...
\item \textbf{\code{ImageIndex GetIndex()}} Returns the \doxygen{itk::Index}
...
reference a single pixel.  The \doxygen{itk::ImageRegionIterator} is the basic,
general purpose ITK image iterator.  The rest of the iterators described in
this section are variations on \doxygen{itk::ImageRegionIterator} and
