...
organized by the different directory components of the module.
...
The \code{itk\_module\_link\_dependencies} macro will link the library to the
libraries defined by the module dependency
specification per Section~\ref{sec:NameAndDependencies}. The \code{itk\_module\_target}
...
Table~\ref{lab:WrappingManglingForPODs},
Table~\ref{lab:WrappingManglingOtherITKPixelTypes}, and
Table~\ref{lab:WrappingManglingITKBasicTypes}. Mangling variables start with the prefix
...
that need one or more image parameter sof the same type. This macro has two
