...
ITK is organized into multiple Git repositories. The library source code are
found in the \code{ITK}\footnote{\url{http://itk.org/ITK.git}} Git repository.
...
\code{InsightApplications}\footnote{\url{http://itk.org/ITKApps.git}} repository.
The sources for this guide are found in the
\code{ITKSoftwareGuide}\footnote{\url{http://itk.org/ITKSoftwareGuide.git}} repository.
Usually, you will work with the \code{ITK} repository.
...
\begin{itemize}
        ...
        to link with the ITK data servers to download test data. This test data
...
\begin{itemize}
        ...
        OpenCV\footnote{\url{http://opencv.org}} or
        ...
        \item \code{ITK/Modules/Remote}---a special group that implements a
        method using CMake to download and include modules from outside the
        ITK Git repository.
        \item \code{ITK/Modules/External}---a special group to place in
        development or non-publicized modules.
...
following commands (under a Git Bash shell):
\begin{verbatim}
