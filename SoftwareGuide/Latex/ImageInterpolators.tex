...
via \doxygen{NearestNeighborInterpolateImageFunction}, linear via
\doxygen{LinearInterpolateImageFunction} and B-spline interpolation via
...
has to trade-off the simplicity of computation with smoothness when selecting
...
\doxygen{NearestNeighborInterpolateImageFunction} simply uses the intensity of
...
\doxygen{LinearInterpolateImageFunction} assumes that intensity varies
linearly between grid positions. Unlike nearest neighbor, the interpolated
intensity is spatially continuous. However, the intensity gradient will be
discontinuous at grid positions.
...
Currently, this interpolator supports spline of order $0$ to $5$. Spline
order of $0$ is almost identical to nearest-neighbor; spline order of $1$ is
exactly identical to linear interpolation. For spline order greater than $1$,
both the interpolated value and their derivative are spatially continuous.
