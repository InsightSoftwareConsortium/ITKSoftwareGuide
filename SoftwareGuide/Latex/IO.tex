...
based on the previous NEMA specification.  It specify a file format definition
as well as a network communication protocol. DICOM was developed to enable
integration of scanners, servers, workstations and network hardware from
...
the domain of medical imaging since most all the images that are acquired a
clinical setting are stored and transported using the DICOM specification.

DICOM functionalities in ITK are provided by the GDCM library. This open source
library was developed by the Creatis Laboratory~\footnote{INSA Lyon:
http://www.www.creatis.insa-lyon.fr}. Although originally this library was
distributed under a LGPL
License\footnote{http://www.gnu.org/copyleft/lesser.html}, the Creatis group
was lucid enough to understand the limitations of that license and agreed to
adopt the more open BSD-like
License\footnote{http://www.opensource.org/licenses/bsd-license.php} that is
used by ITK. This change made possible to distribute GDCM along with ITK.
